...
def _build_pdf(proyecto, secciones, condiciones, moneda="MN"):
    ...
    pdf.set_font('helvetica', '', 9)
    # add 30 days to fecha_creacion for vencimiento (like old logic usually did, or fetch from DB)
    ...
    for s in secciones:
        if s.get("codigo") in ("PRESE", "REPORTE", "CONDICIONES"):
            continue
            
        pdf.set_font('helvetica', 'B', 10)
        pdf.set_text_color(26, 58, 92)
        pdf.cell(0, 8, str(s.get("titulo") or s.get("codigo") or ""), ln=True)
        
        partidas = s.get("partidas", [])
        if not partidas:
            pdf.set_font('helvetica', 'I', 8)
            pdf.set_text_color(100, 100, 100)
            pdf.cell(0, 6, "Sin partidas", ln=True)
            pdf.ln(4)
            continue
    ...
    pdf.cell(115)
    pdf.cell(30, 5, "SUBTOTAL:", ln=0)
    pdf.cell(0, 5, format_currency(subtotal_mn), ln=1, align="R")
    
    pdf.cell(115)
    pdf.cell(30, 5, "IVA (16%):", ln=0)
    pdf.cell(0, 5, format_currency(iva), ln=1, align="R")
    
    pdf.set_font('helvetica', 'B', 11)
    pdf.set_text_color(26, 58, 92)
    pdf.cell(115)
    pdf.cell(30, 8, "TOTAL:", ln=0)
    pdf.cell(0, 8, format_currency(total_final), ln=1, align="R")
    ...
    pdf.set_font_size(7)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 4, "Nota: precios en Pesos Mexicanos MN, precios sujetos a cambio sin previo aviso", ln=True)
    cond_pago = str(proyecto.get('condiciones_pago') or '90 DIAS')
    tiempo_entrega = str(proyecto.get('tiempo_entrega') or '8 DIAS HABILES')
    pdf.cell(0, 4, f"TÉRMINOS Y CONDICIONES: Condiciones de Pago: {cond_pago}  |  Tiempo de Entrega: {tiempo_entrega}", ln=True)
    
    pdf.ln(6)
    pdf.set_font_size(8)
    pdf.set_text_color(40, 40, 40)
    pdf.cell(0, 4, "Para cualquier aclaración con respecto a esta cotización, favor de comunicarse al", ln=True)
    pdf.set_text_color(0, 100, 180)
    pdf.cell(0, 4, "correo integraqro07@outlook.com", ln=True)
    pdf.set_text_color(40, 40, 40)
    pdf.cell(0, 4, "Atención: Jose Moreno Rangel  tel: 447 7214891", ln=True)
...
if __name__ == "__main__":
    from app import q
    # Test project
    proyecto = q("SELECT * FROM proyectos WHERE id=1", fetch="one")
    secciones = q("SELECT * FROM secciones WHERE proyecto_id=1 ORDER BY orden")
    for s in secciones:
        if s["tipo"] == "mano_obra":
    ...
    print("PDF Generated successfully as test_pdf.pdf")
